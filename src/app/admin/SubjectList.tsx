...
import { Edit, Trash2, BookOpen, Search, Filter, ChevronDown, SortAsc, SortDesc,
         ...
         CheckCircle, AlertCircle, Eye, BarChart2, Paperclip, Layers, PenTool, Save } from 'lucide-react'; // Added Save

// Assuming SubjectIcon component exists and works
import SubjectIcon from '@/components/icons/SubjectIcon'; // Adjust path if needed
...
interface Subject {
    ...
    description: string;
    color?: string;
    gradientFrom?: string; // Added for gradient
    gradientTo?: string;   // Added for gradient
    icon?: string;
    topics: Topic[];
    isActive?: boolean;
    createdAt?: string;
...
const SubjectList: React.FC<SubjectListProps> = ({ subjects, onEdit, onDelete, isPreviewMode = false }) => {
  ...
  // Generate a gradient based on subject color and gradient fields
  const generateGradient = (subject: Subject) => {
    const from = subject.gradientFrom || subject.color || '#6b7280';
    const to = subject.gradientTo || subject.color || '#4a5568'; // Default darker gray if no 'to'
  ...
  // Toggle confirmation for delete - PREVENTS action in preview mode
  const handleDeleteClick = (subjectId: string) => {
    if (isPreviewMode) {
      showPreviewModeAlert("Deleting subjects");
      return;
    }
  ...
  if (subjects.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center p-10 bg-white dark:bg-gray-800 rounded-xl shadow-lg border border-gray-200 dark:border-gray-700 text-center transition-all duration-500">
        ...
        <button
          className={`px-4 py-2 bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-700 hover:to-indigo-700 text-white rounded-lg shadow-lg hover:shadow-purple-500/20 transition-all duration-300 flex items-center transform hover:scale-105 ${isPreviewMode ? 'opacity-50 cursor-not-allowed hover:from-purple-600 hover:to-indigo-600 transform-none' : ''}`}
          onClick={() => isPreviewMode ? showPreviewModeAlert("Creating subjects") : {}} // This would typically open your create subject form
          disabled={isPreviewMode}
        >
          {isPreviewMode ? (
            <><Eye className="w-5 h-5 mr-2" /> Preview Mode</>
          ) : (
            <><PlusCircle className="w-5 h-5 mr-2" /> Create First Subject</>
          )}
        </button>
      </div>
    );
            ...
            <div className="flex items-center">
              ...
              <div className="ml-3">
                ...
                <div className="mt-1 text-sm text-amber-700 dark:text-amber-200">
                  You can view subjects but editing functionality is disabled in preview mode.
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="bg-amber-50 dark:bg-amber-900/30 border border-amber-200 dark:border-amber-800 rounded-lg p-4 mb-6 animate-pulse-subtle shadow-md">
          <div className="flex items-center">
            ...
            <div className="ml-3">
              <h3 className="text-sm font-medium text-amber-800 dark:text-amber-300">Preview Mode</h3>
              <div className="mt-1 text-sm text-amber-700 dark:text-amber-200">
                You can view subjects but editing and deleting are disabled.
              </div>
            </div>
          </div>
        </div>
          ...
          {filteredSubjects.map((subject, index) => (
            ...
            >
              ...
              <div
                className="h-3"
                style={{
                  background: generateGradient(subject),
                  boxShadow: `0 2px 6px ${subject.color || '#6b7280'}40`
                }}
              ></div>
              ...
              <div className="p-5">
                ...
                <div className="flex justify-end mt-3 space-x-1">
                  {/* Edit Button - Always visible, calls onEdit */}
                  <button
                    onClick={() => onEdit(subject)} // Always calls onEdit
                    ...
                    title={`Edit ${subject.name}`} // Title remains the same
                  >
                    <Edit className="h-5 w-5" />
                  </button>

                  {/* Delete Button - Visually disabled in preview */}
                  <button
                    onClick={() => handleDeleteClick(subject._id)}
                    disabled={isPreviewMode} // Disable button in preview mode
                    className={`p-1.5 rounded-md transition-colors ${
                      isPreviewMode
                        ? 'text-gray-400 dark:text-gray-500 opacity-50 cursor-not-allowed' // Disabled style
                        : confirmDelete === subject._id
                          ? 'bg-red-100 text-red-600 dark:bg-red-900/30 dark:text-red-400' // Confirm style
                          : 'text-gray-600 hover:text-red-600 hover:bg-red-50 dark:text-gray-400 dark:hover:text-red-400 dark:hover:bg-red-900/20' // Normal style
                    }`}
                    aria-label={isPreviewMode ? "Preview mode - Delete disabled" : (confirmDelete === subject._id ? 'Confirm delete' : `Delete ${subject.name}`)}
                    title={isPreviewMode ? "Preview mode - Delete disabled" : (confirmDelete === subject._id ? 'Confirm delete' : `Delete ${subject.name}`)}
                  >
                    {confirmDelete === subject._id && !isPreviewMode ? ( // Only show confirm icon if not preview
                  ...
                  {/* Duplicate Button - Visually disabled in preview */}
                  <button
                    onClick={() => {
                      if (isPreviewMode) {
                        showPreviewModeAlert("Duplicating subjects");
                        return;
                      }
                      console.log(`Duplicate subject: ${subject.name}`);
                      // Add actual duplicate logic here if needed
                    }}
                    disabled={isPreviewMode} // Disable button in preview mode
                    className={`p-1.5 rounded-md transition-colors ${
                      isPreviewMode
                        ? 'text-gray-400 dark:text-gray-500 opacity-50 cursor-not-allowed' // Disabled style
                        : 'text-gray-600 hover:text-blue-600 hover:bg-blue-50 dark:text-gray-400 dark:hover:text-blue-400 dark:hover:bg-blue-900/20' // Normal style
                    }`}
                    aria-label={isPreviewMode ? "Preview mode - Duplicate disabled" : `Duplicate ${subject.name}`}
                    title={isPreviewMode ? "Preview mode - Duplicate disabled" : `Duplicate ${subject.name}`}
                  >
                      ...
                      <div className="flex items-center">
                        ...
                        <div className="ml-3">
                          ...
                          <div
                            ...
                            style={{ background: generateGradient(subject) }}
                          ></div>
                        </div>
                      </div>
                    </td>
