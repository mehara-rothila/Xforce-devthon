...
import { Loader2, AlertCircle, Check, Layout, Edit } from 'lucide-react'; // <-- Import icons
...
interface ForumCategory {
  ...
  // Style properties added dynamically
  gradientFrom?: string;
  gradientTo?: string;
  lightBg?: string;
  iconName?: string; // Added to store resolved icon name
}

// Enhanced category styling (returns style classes and resolved icon name)
const getCategoryStyles = (categoryName: string = '') => {
    ...
    switch(lowerName) {
      ...
      default: return { color: 'purple', gradientFrom: 'from-purple-400', gradientTo: 'to-purple-600', lightBg: 'bg-purple-50 dark:bg-purple-900/20', iconName: 'chat' }; // Default to chat icon maybe?
    }
};
...
export default function NewTopicPage() {
  ...
  const [isSubmitting, setIsSubmitting] = useState<boolean>(false); // Submitting form
  const [error, setError] = useState<string | null>(null); // Error fetching categories
  const [submitError, setSubmitError] = useState<string | null>(null); // Error submitting form

  // Fetch categories using api.js
  useEffect(() => {
...
const enhancedCategories = fetchedCategories.map((cat: ForumCategory) => ({                ...cat,
    ...
    fetchCategories();
  }, [initialCategoryId]); // Depend on initialCategoryId
  ...
  // Use default styles if category not found yet
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-purple-50 to-gray-100 dark:from-gray-800 dark:to-gray-900 py-8 px-4 sm:px-6 lg:px-8 relative overflow-hidden transition-colors duration-300">
      ...
      <div className="max-w-3xl mx-auto relative z-10">
        ...
        {isLoading && ( <div className="bg-white/90 dark:bg-gray-800/90 backdrop-filter backdrop-blur-sm rounded-xl shadow-xl p-8 mb-8 flex justify-center items-center animate-fadeIn"><Loader2 className="h-8 w-8 animate-spin text-purple-600 dark:text-purple-400" /><span className="ml-3 text-lg text-gray-600 dark:text-gray-400">Loading categories...</span></div> )}
        {error && ( <div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-xl p-6 mb-8 animate-fadeIn"><div className="flex items-center"><AlertCircle className="h-6 w-6 text-red-600 dark:text-red-400 mr-3" /><p className="text-red-800 dark:text-red-200">{error}</p></div><button onClick={() => window.location.reload()} className="mt-4 px-4 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700 dark:bg-red-700 dark:hover:bg-red-600 transition-colors duration-300"> Try Again </button></div> )}
        ...
        {!isLoading && !error && (
          <div className="bg-white/90 dark:bg-gray-800/90 backdrop-filter backdrop-blur-sm rounded-xl shadow-xl overflow-hidden animate-fadeIn hover:shadow-2xl transition-all duration-300">
            ...
            <div className="p-6 border-b border-gray-100 dark:border-gray-700">
              <h1 className="text-2xl font-bold text-gray-900 dark:text-gray-100 flex items-center text-shadow"> <Layout className="h-6 w-6 mr-3 text-purple-500 dark:text-purple-400" /> Create a New Topic </h1>
            ...
            <div className="p-6">
              <form onSubmit={handleSubmit}>
                {/* Title input */}
                <div className="mb-6">
                  <label htmlFor="title" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2"> Topic Title <span className="text-red-500">*</span> </label>
                  <div className="relative">
                    <input type="text" id="title" name="title" className="w-full p-3 pl-10 border border-gray-300 dark:border-gray-600 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-purple-500 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 transition-colors duration-200 shadow-sm hover:shadow focus:shadow-md" placeholder="Enter a clear, specific title for your topic" value={title} onChange={(e) => setTitle(e.target.value)} disabled={isSubmitting} required />
                    <div className="absolute top-3 left-3 text-gray-400 dark:text-gray-500"> <Edit className="h-5 w-5" /> </div>
                  </div>
                ...
                <div className="mb-6">
                  ...
                  <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                    {categories.map((cat, index) => {
                       // Use style info added during fetch
                       const catStyle = cat as ForumCategory & { color: string; lightBg: string; iconName: string };
                       return (
                         <div key={cat._id} 
                              className={`p-3 border rounded-lg cursor-pointer transition-all duration-300 flex items-center animate-fadeIn transform hover:scale-105 hover:shadow-md ${ selectedCategory === cat._id ? `${catStyle.lightBg} border-${catStyle.color}-300 shadow-sm dark:bg-${catStyle.color}-900/20 dark:border-${catStyle.color}-700` : 'border-gray-200 dark:border-gray-700 hover:border-gray-300 dark:hover:border-gray-600 hover:bg-gray-50 dark:hover:bg-gray-700/50' }`} 
                              style={{ animationDelay: `${index * 0.05}s` }}
                              onClick={() => setSelectedCategory(cat._id)} >
                           <div className={`p-2 mr-3 rounded-md ${selectedCategory === cat._id ? `bg-${catStyle.color}-100 text-${catStyle.color}-600 dark:bg-${catStyle.color}-900/30 dark:text-${catStyle.color}-400` : 'bg-gray-100 dark:bg-gray-700 text-gray-500 dark:text-gray-400'}`}>
                             {/* Use SubjectIcon with iconName from enhanced category */}
                             <SubjectIcon iconName={catStyle.iconName || 'book'} className="h-5 w-5" color={selectedCategory === cat._id ? catStyle.color : undefined}/>
                           </div>
                           <div> <span className={`font-medium ${selectedCategory === cat._id ? `text-${catStyle.color}-700 dark:text-${catStyle.color}-400` : 'text-gray-700 dark:text-gray-300'}`}> {cat.name} </span> </div>
                         </div>
                       );
                    })}
                  </div>
                ...
                <div className="flex border-b border-gray-200 dark:border-gray-700 mb-0">
                  <button type="button" className={`py-2 px-4 font-medium text-sm ${ activeTab === 'write' ? 'text-purple-600 dark:text-purple-400 border-b-2 border-purple-600 dark:border-purple-400' : 'text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-gray-200' } transition-colors duration-200`} onClick={() => setActiveTab('write')} > Write </button>
                  <button type="button" className={`py-2 px-4 font-medium text-sm ${ activeTab === 'preview' ? 'text-purple-600 dark:text-purple-400 border-b-2 border-purple-600 dark:border-purple-400' : 'text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-gray-200' } transition-colors duration-200`} onClick={() => setActiveTab('preview')} disabled={!content} > Preview </button>
                </div>
                ...
                {activeTab === 'write' && ( 
                  <div className="flex items-center border-x border-t border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-700/50 px-4 py-2 text-sm rounded-t-lg">
                    <div className="flex space-x-2">
                      <button type="button" className="p-1.5 rounded hover:bg-gray-200 dark:hover:bg-gray-600 text-gray-600 dark:text-gray-300" title="Bold" onClick={() => setContent(prev => prev + '**bold text**')}>
                        <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 10V3L4 14h7v7l9-11h-7z" />
                        </svg>
                      </button>
                      <button type="button" className="p-1.5 rounded hover:bg-gray-200 dark:hover:bg-gray-600 text-gray-600 dark:text-gray-300" title="Italic" onClick={() => setContent(prev => prev + '*italic text*')}>
                        <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M7 8h10M7 12h4m1 8l-4-4H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-3l-4 4z" />
                        </svg>
                      </button>
                      <button type="button" className="p-1.5 rounded hover:bg-gray-200 dark:hover:bg-gray-600 text-gray-600 dark:text-gray-300" title="Code" onClick={() => setContent(prev => prev + '`code`')}>
                        <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                <div className="mb-6">
                  {activeTab === 'write' ? (
                    <textarea id="content" name="content" rows={10} className="w-full p-4 border border-gray-300 dark:border-gray-600 rounded-b-lg focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-purple-500 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 placeholder-gray-500 dark:placeholder-gray-400 transition-all duration-200 shadow-inner hover:shadow-md focus:shadow-lg" placeholder="Describe your question or topic..." value={content} onChange={(e) => setContent(e.target.value)} disabled={isSubmitting} required ></textarea>
                  ) : (
                    <div className="w-full p-4 min-h-[268px] border border-gray-300 dark:border-gray-600 rounded-b-lg bg-gray-50 dark:bg-gray-700/50 prose dark:prose-invert prose-sm max-w-none animate-fadeIn">
                      {content ? ( <div dangerouslySetInnerHTML={renderPreview()} /> ) : ( <p className="text-gray-400 dark:text-gray-500 italic">Nothing to preview yet.</p> )}
                    </div>
                  )}
                ...
                {/* Submit error */}
                {submitError && ( <div className="mb-6 p-3 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg text-red-800 dark:text-red-200 text-sm animate-fadeIn"> {submitError} </div> )}

                {/* Submit buttons */}
                <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
                  <div className="text-sm text-gray-600 dark:text-gray-400 bg-yellow-50 dark:bg-yellow-900/20 p-3 rounded-lg border border-yellow-100 dark:border-yellow-800 flex items-start hover:shadow-md transition-all duration-200 transform hover:scale-101"> <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 text-yellow-500 dark:text-yellow-400 mr-2 flex-shrink-0 mt-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor"> <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" /> </svg> <span> <span className="font-medium">Tip:</span> Be specific in your question to get better answers. </span> </div>
                  <div className="flex space-x-3 self-end">
                    <Link href="/forum" className="px-5 py-2.5 border border-gray-300 dark:border-gray-600 text-gray-700 dark:text-gray-300 rounded-lg font-medium hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors duration-200 flex items-center hover:shadow transform hover:translate-y-[-2px] active:translate-y-[1px]"> <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor"> <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10 19l-7-7m0 0l7-7m-7 7h18" /> </svg> Cancel </Link>
                    <button type="submit" className={`px-5 py-2.5 bg-gradient-to-r from-purple-500 to-purple-700 hover:from-purple-600 hover:to-purple-800 dark:from-purple-600 dark:to-purple-800 dark:hover:from-purple-700 dark:hover:to-purple-900 text-white rounded-lg font-medium shadow-md hover:shadow-lg transition-all duration-300 flex items-center transform hover:translate-y-[-2px] active:translate-y-[1px] ${ !title || !selectedCategory || !content || isSubmitting ? 'opacity-70 cursor-not-allowed' : '' }`} disabled={!title || !selectedCategory || !content || isSubmitting} >
                      {isSubmitting ? ( <><Loader2 className="animate-spin -ml-1 mr-2 h-4 w-4 text-white" /> Creating Topic...</> ) : ( <><svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor"> <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 19l9 2-9-18-9 18 9-2zm0 0v-8" /> </svg> Create Topic</> )}
                    </button>
                  </div>
                </div>
              </form>
            ...
            <div className="p-6 bg-gradient-to-r from-gray-50 to-white dark:from-gray-800 dark:to-gray-750 border-t border-gray-100 dark:border-gray-700">
                {/* Posting Guidelines */}
                 <div className="flex items-center mb-3"> <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 text-purple-500 dark:text-purple-400 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor"> <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z" /> </svg> <h3 className="text-sm font-medium text-gray-800 dark:text-gray-200 text-shadow">Posting Guidelines:</h3> </div> <ul className="text-sm text-gray-600 dark:text-gray-400 space-y-2 ml-7"> <li className="flex items-start group hover:text-gray-800 dark:hover:text-gray-200 transition-colors duration-200"> <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 text-purple-500 dark:text-purple-400 mr-2 flex-shrink-0 mt-0.5 group-hover:scale-110 transition-transform duration-200" fill="none" viewBox="0 0 24 24" stroke="currentColor"> <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" /> </svg> <span>Make sure your question hasn't been asked before by using the search function.</span> </li> <li className="flex items-start group hover:text-gray-800 dark:hover:text-gray-200 transition-colors duration-200"> <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 text-purple-500 dark:text-purple-400 mr-2 flex-shrink-0 mt-0.5 group-hover:scale-110 transition-transform duration-200" fill="none" viewBox="0 0 24 24" stroke="currentColor"> <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" /> </svg> <span>Be clear and concise in your title and description.</span> </li> <li className="flex items-start group hover:text-gray-800 dark:hover:text-gray-200 transition-colors duration-200"> <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 text-purple-500 dark:text-purple-400 mr-2 flex-shrink-0 mt-0.5 group-hover:scale-110 transition-transform duration-200" fill="none" viewBox="0 0 24 24" stroke="currentColor"> <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" /> </svg> <span>For math equations, use proper formatting (surround with $ for inline or $$ for block).</span> </li> <li className="flex items-start group hover:text-gray-800 dark:hover:text-gray-200 transition-colors duration-200"> <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 text-purple-500 dark:text-purple-400 mr-2 flex-shrink-0 mt-0.5 group-hover:scale-110 transition-transform duration-200" fill="none" viewBox="0 0 24 24" stroke="currentColor"> <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" /> </svg> <span>Be respectful and considerate of other community members.</span> </li> </ul>
            </div>
          </div>
        )}
        ...
        {!isLoading && !error && (
          <div className="mt-6 bg-white/90 dark:bg-gray-800/90 backdrop-filter backdrop-blur-sm rounded-xl shadow-xl overflow-hidden animate-fadeIn hover:shadow-2xl transition-all duration-300" style={{ animationDelay: '0.2s' }}>
             {/* Quick Help content */}
             <div className="p-6 border-b border-gray-100 dark:border-gray-700 flex items-center bg-gradient-to-r from-gray-50 to-white dark:from-gray-800 dark:to-gray-750"> <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 text-purple-500 dark:text-purple-400 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor"> <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /> </svg> <h2 className="text-lg font-bold text-gray-900 dark:text-gray-100 text-shadow">Quick Help</h2> </div> 
             <div className="p-6"> 
      ...
      <style jsx global>{`
        ...
        *:focus-visible {
        ...
        /* New Topic page specific styles */
        ...
        /* Category selection hover effects */
        ...
        /* Special animation for width grows */
        @keyframes widthGrow { 
          from { width: 0%; } 
