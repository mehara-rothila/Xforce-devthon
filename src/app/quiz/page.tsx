...
import { useAuth } from '@/app/context/AuthContext'; // Correct import
...
export default function Quizzes() {
  ...
  useEffect(() => {
    const fetchQuizzes = async () => {
      ...
      setError(null);
      try {
        const params: Record<string, string> = { isPublished: 'true' };
        if (activeSubjectId !== 'all') params.subject = activeSubjectId;
        if (activeDifficulty !== 'all') params.difficulty = activeDifficulty;

        // Map user-friendly sort options to API sort parameters
        switch (sortBy) {
            case 'popular': params.sort = '-attempts'; break;
            case 'newest': params.sort = '-createdAt'; break;
            case 'rating': params.sort = '-rating'; break;
            case 'easy-to-hard': params.sort = 'difficulty'; break; // Sort ascending for easy first
            case 'hard-to-easy': params.sort = '-difficulty'; break;// Sort descending for hard first
            default: params.sort = '-attempts'; // Default to popular
        }

        const response = await api.quizzes.getAll(params);
        const fetchedQuizzes = response.data?.data?.quizzes || [];

        // Ensure data is an array and map correctly
        if (Array.isArray(fetchedQuizzes)) {
          setQuizzes(fetchedQuizzes.map(q => ({
              ...q,
              // Provide fallback subject if necessary
              subject: q.subject || { _id: 'unknown', name: 'Unknown' },
              // Use virtual or calculate totalQuestions safely
        ...
        setError(`Failed to load quizzes: ${err.response?.data?.message || err.message || 'Please try again later.'}`);
        setQuizzes([]);
      } finally {
        setIsLoading(false);
      }
  ...
  useEffect(() => {
    const fetchUserStats = async () => {
      if (isAuthLoading) {
         ...
         setStatsLoading(true); // Ensure loading state while checking auth
         return; // Wait for auth loading to complete
      }

      // If auth is loaded and user exists, fetch their stats
      if (user?._id) {
        ...
        try {
          ...
          if (summaryData) {
            // Extract stats safely with nullish coalescing
            const completed = summaryData.quizStats?.completed ?? 0;
            const avgScore = summaryData.quizStats?.avgScore ?? 0;
            // Use quiz-specific points first, fallback to stats-level points
            const quizPoints = summaryData.quizPointsEarned ?? summaryData.quizStats?.pointsEarned ?? 0;

            console.log(`[QuizPage Stats] Setting state: completed=${completed}, accuracy=${avgScore}, quizPoints=${quizPoints}`);
            setDynamicStats({
              completed: completed,
              accuracy: avgScore,
              points: quizPoints // Use the extracted quizPoints value
            });
          } else {
            console.error("[QuizPage Stats] Summary data missing or invalid in API response");
          ...
          setDynamicStats(null); // No stats to display
          setStatsError(null); // No error fetching (just not logged in)
      }
    };

    fetchUserStats();
    // Dependency array includes user and isAuthLoading to re-fetch when auth state changes
  }, [user, isAuthLoading]);
  ...
  const getSubjectClass = (subject: QuizSubject | undefined | null): string => {
    // Fallback style if subject is missing
    if (!subject) return isDarkMode ? 'bg-gray-700 text-gray-300 border border-gray-600' : 'bg-gray-100 text-gray-800 border border-gray-200';
  ...
  const getActiveSubjectClass = (subjectName: string | undefined | null): string => {
    switch (subjectName?.toLowerCase()) {
      ...
      // Default active style (for "All Subjects" or others)
  ...
  const getInactiveSubjectClass = (subjectName: string | undefined | null): string => {
    // Base inactive style (for "All Subjects" if inactive or unknown subjects)
    ...
    switch (subjectName?.toLowerCase()) {
      ...
      // Use base style if subject doesn't match specific cases
  ...
  const progressPercent = dynamicStats ? Math.min(100, Math.max(0, Math.round((dynamicStats.completed / 50) * 100))) : 0;
  ...
  return (
    <div className={`min-h-screen ${isDarkMode ? 'bg-gradient-to-b from-gray-900 to-gray-800' : 'bg-gradient-to-b from-gray-50 to-gray-100'} transition-colors duration-300 relative overflow-hidden`}>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 lg:-mt-16 pb-12 relative z-10"> {/* Corrected mt for overlap */}

        {/* Filters Card */}
        <div className={`${isDarkMode ? 'bg-gray-800/90 border-gray-700' : 'bg-white/90 border-gray-100'} rounded-2xl shadow-xl overflow-hidden mb-8 transition-colors duration-300 border backdrop-blur-sm animate-fade-in`}>
          <div className="p-6">
            <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  <button onClick={() => handleSubjectFilter('all')} className={`px-4 py-2 rounded-lg text-sm font-medium transition-all duration-200 ${
                    activeSubjectId === 'all' ? getActiveSubjectClass(null) : getInactiveSubjectClass(null)}`}>
                    All Subjects
                  </button>
                  {availableSubjects.map((subject) => (
                    <button key={subject._id} onClick={() => handleSubjectFilter(subject._id)} className={`px-4 py-2 rounded-lg text-sm font-medium transition-all duration-200 ${
                      activeSubjectId === subject._id ? getActiveSubjectClass(subject.name) : getInactiveSubjectClass(subject.name)}`}>
                      {subject.name}
                    </button>
                  ))}
                </div>
                   ...
                   {['all', 'easy', 'medium', 'hard'].map((diff) => {
                     const isActive = activeDifficulty === diff;
                      ...
                      if (diff === 'all') {
                          ...
                          inactiveClass = isDarkMode ? 'bg-green-900/30 text-green-400 hover:bg-green-800/50 border border-green-800/50' : 'bg-green-50 text-green-700 hover:bg-green-100 border border-green-100';
                      } else if (diff === 'medium') {
                          activeClass = 'bg-gradient-to-r from-yellow-500 to-yellow-600 text-white shadow-md'; 
                          inactiveClass = isDarkMode ? 'bg-yellow-900/30 text-yellow-400 hover:bg-yellow-800/50 border border-yellow-800/50' : 'bg-yellow-50 text-yellow-700 hover:bg-yellow-100 border border-yellow-100';
                      } else if (diff === 'hard') {
                           activeClass = 'bg-gradient-to-r from-red-500 to-red-600 text-white shadow-md'; 
                           inactiveClass = isDarkMode ? 'bg-red-900/30 text-red-400 hover:bg-red-800/50 border border-red-800/50' : 'bg-red-50 text-red-700 hover:bg-red-100 border border-red-100';
                      }
        ...
        {!isLoading && !error && quizzes.length > 0 && (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mb-12">
            {quizzes.map((quiz, index) => {
              ...
              const isLoggedIn = !isAuthLoading && user; // True if auth is loaded and user object exists
              const isDisabled = isAuthLoading || !isLoggedIn; // Disable if loading or logged out
                ...
                >
                  <div className="p-6">
                    ...
                    <div className="flex justify-end">
                        ...
                        <Link
                          href={isLoggedIn ? `/quiz/take?id=${quiz._id}` : '#'} // Dynamic href
                          className={`
                            px-4 py-2 rounded-lg text-sm font-medium transition-all duration-200 shadow-md flex items-center justify-center
                            ${isDisabled
                              ? 'bg-gray-400 dark:bg-gray-600 text-gray-700 dark:text-gray-400 cursor-not-allowed opacity-70' // Disabled state styles
                              : 'bg-gradient-to-r from-purple-600 to-indigo-600 text-white hover:from-purple-700 hover:to-indigo-700 transform hover:scale-105' // Enabled state styles
                            }
                          `}
                          aria-disabled={isDisabled} // Accessibility
                          onClick={(e) => {
                            if (isDisabled) {
                              e.preventDefault(); // Important: Prevent navigation if disabled
                              console.log("Start Quiz clicked while disabled/logged out.");
                              // Redirect to login only if auth loading is complete and user is confirmed absent
                              if (!isAuthLoading && !user) {
                                // Optionally add a query parameter to redirect back after login
                                router.push('/login?redirect=/quiz');
                              }
                              // If isAuthLoading is true, do nothing - let the button appear disabled
                            }
                            // If logged in (!isDisabled), allow default Link behavior (navigation)
                          }}
                        >
                          {/* Conditional Icon */}
                          {isDisabled ? (
                          ...
                          {/* Conditional Text */}
                          {isAuthLoading ? 'Checking Auth...' : isDisabled ? 'Log in to Start' : 'Start Quiz'}
                        </Link>
                    </div>
                  </div>
                </div>
                ...
                <>
                  ...
                  <div>
                    <div className={`flex justify-between items-center mb-1`}>
                      <span className={`text-sm font-medium ${isDarkMode ? 'text-gray-300' : 'text-gray-700'}`}>Quizzes Completed</span>
                    ...
                    <div className={`h-2 w-full ${isDarkMode ? 'bg-gray-700' : 'bg-gray-200'} rounded-full overflow-hidden`}>
                      <div className="h-2 bg-purple-600 rounded-full" style={{ width: `${Math.min(100, (dynamicStats.completed / 50) * 100)}%` }}></div>
                    </div>
                  </div>
                  ...
                  <div>
                    <div className={`flex justify-between items-center mb-1`}>
                      <span className={`text-sm font-medium ${isDarkMode ? 'text-gray-300' : 'text-gray-700'}`}>Accuracy Rate</span>
                  ...
                  {/* Quiz Points Earned (Updated to use quiz-specific points) */}
                  <div>
                    <div className={`flex justify-between items-center mb-1`}>
                      <span className={`text-sm font-medium ${isDarkMode ? 'text-gray-300' : 'text-gray-700'}`}>Quiz Points Earned</span>
                      <span className="text-sm font-semibold text-purple-700">{dynamicStats.points}</span>
                    </div>
                    <div className={`h-2 w-full ${isDarkMode ? 'bg-gray-700' : 'bg-gray-200'} rounded-full overflow-hidden`}>
                      <div className="h-2 bg-purple-600 rounded-full" style={{ width: `${Math.min(100, (dynamicStats.points / 2000) * 100)}%` }}></div>
                    </div>
                  </div>
                </>
              ...
              <div className="space-y-4">
                 {recentAchievements.map((ach) => (
                   <div key={ach.id} className="flex items-center">
                      ...
                      <div className="ml-3">
                         <h4 className={`text-sm font-medium ${isDarkMode ? 'text-gray-100' : 'text-gray-900'}`}>{ach.title}</h4>
                         <p className={`text-xs ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>{ach.description}</p>
                      </div>
                   </div>
                 ))}
              </div>
           ...
           <div className={`${isDarkMode ? 'bg-gray-800/90 border-gray-700 hover:border-purple-800' : 'bg-white/90 border-gray-100 hover:border-purple-200'} rounded-2xl shadow-lg p-6 border hover:shadow-xl transition-all duration-300 backdrop-blur-sm`}>
               ...
               <div className="space-y-4">
                  {subjectMastery.map((sub) => (
                     <div key={sub.id}>
                         <div className={`flex justify-between items-center mb-1`}>
                            <span className={`text-sm font-medium ${isDarkMode ? 'text-gray-300' : 'text-gray-700'}`}>{sub.name}</span>
